<?php
	...
	/*
		...
		5th version revised by mr.xkr Nov/2019.
	...
	*/
	...
	class xImage {
		...
		function getFormatByMagic($bytes) {
		...
		function getMimeByFormat($format="jpg") {
			switch ($format) {
		...
		function getMimeByMagic($bytes) {
			return $this->getMimeByFormat($this->getFormatByMagic($bytes));
		}
		...
		function getFileFormat($filename) {
			if ($f=(@fopen($filename, "r"))) {
				$bytes=fread($f, 4);
				fclose($f);
				return $this->getFormatByMagic($bytes);
			}
			return false;
		}
		...
		function getFileMime($filename) {
			if ($format=$this->getFileFormat($filename)) return $this->getMimeByFormat($format);
			return false;
		}
