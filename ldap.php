	...
	class LDAP {
		...
		function connect() {
			...
			if ($this->setup["authstring"] && $this->setup["authpass"]) {
				...
				if ($this->ldaperrno=ldap_errno($this->ds))
					$this->ldaperror=ldap_error($this->ds);
				if (!$retv) {
					ldap_close($this->ds);
		...
		function isConnected() { return $this->isconnected; }
		...
		function auth($usr,$pwd) {
			...
			// buscar usuario
			$sr=(@ldap_search($this->ds,$this->setup["querystring"], $this->setup["userquery"].$usr));
			if (!$sr) {
			...
			if ($info["count"]==1) {
				...
				$retv=(@ldap_bind($this->ds, $info[0]["dn"], $pwd)?1:0);
				if ($this->ldaperrno=ldap_errno($this->ds))
					$this->ldaperror=ldap_error($this->ds);
				if ($retv) {
		...
		// devolver códigos de error de LDAP
		function ldap_errno() { return $this->ldaperrno; }
		function ldap_error() { return $this->ldaperror; }
